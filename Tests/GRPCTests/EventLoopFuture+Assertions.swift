...
extension EventLoopFuture {
  ...
  func assertError(
    ...
    file: StaticString = #filePath,
    line: UInt = #line,
    handler: @Sendable @escaping (Error) -> Void = { _ in }
  ) {
    self.whenComplete { result in
      defer {
        expectation.fulfill()
...
extension EventLoopFuture {
  ...
  func peekError(callback: @Sendable @escaping (Error) -> Void) -> EventLoopFuture<Value> {
    self.whenFailure(callback)
    return self
  }

  // TODO: Replace with `always` once https://github.com/apple/swift-nio/pull/981 is released.
  func peek(callback: @Sendable @escaping (Value) -> Void) -> EventLoopFuture<Value> {
